...
import { HADITS_DATA } from '@/constants/database';
...
export default function HaditsDetailScreen() {
  ...
  const hadits = HADITS_DATA.find((item) => item.id === id);
  ...
  if (!hadits) {
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
        <Stack.Screen options={{ title: 'Hadits Tidak Ditemukan', headerShown: true }} />
        <View style={styles.errorContainer}>
          <Text style={{ color: colors.text }}>Hadits tidak ditemukan.</Text>
  ...
  const isFav = isFavorite(hadits.id);

  const handleFavoriteToggle = async () => {
    if (isFav) {
      await removeFavorite(hadits.id);
    } else {
      await addFavorite(hadits.id);
    }
  };

  const handleShare = async () => {
    try {
      const shareMessage = `*${hadits.title}*\n\n${hadits.arab}\n\n_Latin:_\n${hadits.latin}\n\n_Artinya:_\n"${hadits.translation}"\n\n_Referensi: ${hadits.reference}_`;
      await Share.share({
        message: shareMessage,
      });
    } catch (error) {
      Alert.alert('Gagal Berbagi', 'Tidak dapat membagikan hadits.');
    }
  };
              ...
              {hadits.reference && (
                <View style={[styles.referenceContainer, { borderTopColor: colors.border }]}>
                  <Text style={[styles.referenceText, { color: colors.textMuted }]}>
                    Sumber/Referensi: {hadits.reference}
                  </Text>
                </View>
              )}
            </View>
